...
impl Contract {
    ...
    #[payable]
    pub fn clear_nft_series(&mut self) {
        assert_eq!(
            "freehorsesspartans.freehorses.near".to_string(),
    ...
    #[payable]
    pub fn clear_nft_serie(&mut self, token_series_id: TokenSeriesId) {
        assert_eq!(
            "freehorsesspartans.freehorses.near".to_string(),
            env::predecessor_account_id().to_string(),
            "Token owner only"
    ...
    pub fn nft_series_update(
        ...
        token_series.metadata.title = Some(title);
        token_series.metadata.description = Some(description);

        if extra.is_some() {
            token_series.metadata.extra = Some(extra.unwrap());
        }
    ...
    pub fn nft_mint(
        ...
        //let token_id: TokenId = self._nft_mint_series(token_series_id, receiver_id.to_string());
        self._nft_mint_series2(token_series_id, receiver_id.to_string(), token_id.clone());
    ...
    fn _nft_mint_series2(
       ...
       /* let max_copies = token_series.metadata.copies.unwrap_or(u64::MAX);
        ...
        /*token_series.tokens.insert(&token_id);
        self.token_series_by_id.insert(&token_series_id, &token_series);*/
        let title: String = format!("{} {} {}", token_series.metadata.title.unwrap().clone(), TITLE_DELIMETER, token_id.split(':').collect::<Vec<&str>>()[1]);
